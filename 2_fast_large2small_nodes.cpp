...
using namespace fast_l2s_vertex;
...
void fast_l2s_vertex::Foo(std::string codeName, int depth) {
	//Import data to cpp
	...
	for (int x2 = 0; x2 < n + k; x2++)
	{
		...
		{
			if (vis[y2] == false && g.NodeSize(y2) > 1)
			{
				if (g.NodeSize(y2) > temp_max)
				{
					temp_max_idx = y2;
